...
contract("GamePlayer", accounts => {
  ...
  it("can create game player", async () => {
    ...
    assert.strictEqual(
      await player.Name.call(),
      "Slash",
      "Name should be Slash"
    );
    assert.strictEqual(
      await player.Game.call(),
      store.address,
      "expected Game address"
    );
  });
  ...
  it("player can store and discard items", async () => {
    ...
    assert.equal(
      discarded.logs[0].event,
      "ItemDiscarded",
      "event should fire."
    );
    assert.equal(
      discarded.logs[0].args.itemAddress,
      item.address,
  ...
  it("player can store and discard multiple items", async () => {
    ...
    const item2 = await GameItem.new(
      store.address,
      "item1",
      "weapon-sword",
      "{game:data}"
