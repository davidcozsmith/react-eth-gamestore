...
contract("GamePlayer  ", accounts => {
  ...
  before(async () => {
    ...
    assert.equal(
      gameAccounts[0],
      accounts[0],
      "should have a game account registered"
    );
  ...
  it("ANYONE:can get game item props", async () => {
    ...
    assert.strictEqual(
      utils.hexToNumber(props.gameState),
      ItemGameState.NotRegistered,
      "expected not registred"
    );
  ...
  it("NOT-OWNER:ChangeOwner:revert - not the owner", async () => {
    ...
    await assert.reverts(
      item.ChangeOwner(accounts[1], { from: accounts[2] }),
      "Must be owner"
    );
  });
  ...
  it("NOT-OWNER:SetPrice:revert - not the owner", async () => {
    ...
    await assert.reverts(
      item.SetPrice(200, { from: accounts[2] }),
      "Must be owner"
    );
  });
  ...
  it("Game:GameChangeOwner:revert - not the game", async () => {
    ...
    await assert.reverts(
      item.GameChangeOwner(accounts[3], { from: accounts[2] }),
      "Must be the GameStore"
    );
  });
  ...
  it("Game:GameItemRegistered:succeed", async () => {
    ...
    assertEnum(
      ItemGameState.Registered,
      await get(item, "gameState", true),
      "Registered"
    );
  });
  it("Game:GameItemRegistered:revert - not the game", async () => {
    ...
    await assert.reverts(
      item.GameItemRegistered({ from: accounts[2] }),
      "Must be the GameStore"
    );
  });
